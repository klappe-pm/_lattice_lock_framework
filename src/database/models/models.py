...
class Model(Base, UUIDMixin, TimestampMixin):
    ...
    # Classification
    category: Mapped[Optional[str]] = mapped_column(String(50))  # 'reasoning', 'coding', 'creative', etc.
    ...
    # Cost (per 1K tokens)
    input_cost_per_1k: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 6))
    output_cost_per_1k: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 6))
    
    # Status
    status: Mapped[str] = mapped_column(String(20), default="active")  # 'active', 'deprecated', 'beta'
    deprecation_date: Mapped[Optional[date]] = mapped_column(Date)
...
class ProviderCredential(Base, UUIDMixin, TimestampMixin):
    ...
    aws_region: Mapped[Optional[str]] = mapped_column(String(50))
    aws_access_key_encrypted: Mapped[Optional[bytes]] = mapped_column(LargeBinary)
    aws_secret_key_encrypted: Mapped[Optional[bytes]] = mapped_column(LargeBinary)
    
    # Azure specific
    azure_endpoint: Mapped[Optional[str]] = mapped_column(Text)
    azure_deployment_name: Mapped[Optional[str]] = mapped_column(String(255))
    
    # Status
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    last_validated_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
    validation_status: Mapped[Optional[str]] = mapped_column(String(20))  # 'valid', 'invalid', 'pending'
...
class OrganizationQuota(Base, UUIDMixin, TimestampMixin):
    ...
    daily_token_limit: Mapped[Optional[int]] = mapped_column(BigInteger)
    
    # Request limits
    requests_per_minute: Mapped[int] = mapped_column(Integer, default=100)
    requests_per_day: Mapped[Optional[int]] = mapped_column(Integer)
    
    # Cost limits
    monthly_cost_limit: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 2))
    daily_cost_limit: Mapped[Optional[Decimal]] = mapped_column(Numeric(10, 2))
    
    # Model access control
    allowed_models: Mapped[Optional[list[str]]] = mapped_column(ARRAY(Text))  # NULL = all models
    ...
    period_reset_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True))
...
class UserQuota(Base, UUIDMixin, TimestampMixin):
    ...
    monthly_token_limit: Mapped[Optional[int]] = mapped_column(BigInteger)
    daily_token_limit: Mapped[Optional[int]] = mapped_column(BigInteger)
    requests_per_minute: Mapped[Optional[int]] = mapped_column(Integer)
