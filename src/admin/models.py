...
class Project(Base):
    ...
    status: Mapped[ProjectStatus] = mapped_column(
        SQLEnum(ProjectStatus), default=ProjectStatus.UNKNOWN
    )
    registered_at: Mapped[float] = mapped_column(
        Float, default=lambda: datetime.utcnow().timestamp()
    )
    last_activity: Mapped[float] = mapped_column(
        Float, default=lambda: datetime.utcnow().timestamp()
    )

    # Validation fields
    schema_status: Mapped[ValidationStatus] = mapped_column(
        SQLEnum(ValidationStatus), default=ValidationStatus.NOT_RUN
    )
    sheriff_status: Mapped[ValidationStatus] = mapped_column(
        SQLEnum(ValidationStatus), default=ValidationStatus.NOT_RUN
    )
    gauntlet_status: Mapped[ValidationStatus] = mapped_column(
        SQLEnum(ValidationStatus), default=ValidationStatus.NOT_RUN
    )
    last_validated: Mapped[float | None] = mapped_column(Float, nullable=True)
    validation_errors: Mapped[list[str]] = mapped_column(JSON, default=list)

    metadata_json: Mapped[dict] = mapped_column(JSON, default=dict)

    errors: Mapped[list["ProjectError"]] = Relationship(
        back_populates="project", cascade="all, delete-orphan"
    )
    rollback_checkpoints: Mapped[list["RollbackCheckpoint"]] = Relationship(
        back_populates="project", cascade="all, delete-orphan"
    )
