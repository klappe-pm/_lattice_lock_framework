...
class Normalizer:
    ...
    def _denormalize_dict_of_dicts(
        ...
        for record in records:
            ...
            item_data = {k: v for k, v in record.items() if k not in ("_id", "name")}

            # Restore child data
            record_id = record.get("_id")
            for child_table_name in child_table_names:
                child_data = all_data.get(child_table_name, [])
                related = [r for r in child_data if r.get(f"{table_name}_id") == record_id]
                ...
                if related:
                    ...
                    if all("field_name" in r for r in related):
                        ...
                        item_data[field_name] = {
                            r["field_name"]: self._extract_child_value(r) for r in related
                        }
                    else:
                        # Array pattern
                        item_data[field_name] = [self._extract_child_value(r) for r in related]
...
class AgentNormalizer(Normalizer):
    ...
    def normalize_agents(self, agents_data: dict | list) -> dict:
        ...
        if isinstance(agents_data, dict):
            items = [(name, data) for name, data in agents_data.items()]
        else:
            items = [(a.get("name", f"agent_{i}"), a) for i, a in enumerate(agents_data)]
        ...
        for name, agent_data in items:
            ...
            directive = agent_data.get(
                "directive", agent_data.get("agent", {}).get("directive", {})
            )
            scope = agent_data.get("scope", agent_data.get("agent", {}).get("scope", {}))
            delegation = agent_data.get(
                "delegation", agent_data.get("agent", {}).get("delegation", {})
            )
