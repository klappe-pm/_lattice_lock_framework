...
def validate_env_file(file_path: str, required_vars: Optional[List[str]] = None) -> ValidationResult:
    ...
    for i, line in enumerate(lines):
        ...
        if not re.match(r'^[A-Z][A-Z0-9_]*$', key):
            result.add_warning(f"Variable '{key}' does not follow UPPER_SNAKE_CASE convention", line_number=line_num)
        ...
        if is_secret_key:
            ...
            if not is_placeholder:
                ...
                result.add_error(f"Potential plaintext secret detected in '{key}'", line_number=line_num)
