...
def validate_lattice_schema(file_path: str) -> ValidationResult:
    ...
    if "version" in data:
        ...
        if not re.match(r"^v\d+\.\d+(?:\.\d+)?$", version):
            result.add_error(
                f"Invalid version format: {version}. Must be vX.Y or vX.Y.Z", field_path="version"
            )
    ...
    if "entities" in data and isinstance(data["entities"], dict):
        for entity_name, entity_def in data["entities"].items():
            if "ensures" in entity_def:
                _validate_ensures(
                    entity_name, entity_def["ensures"], entity_def.get("fields", {}), result
                )

    return result


def _validate_ensures(
    entity_name: str, ensures_def: Any, fields_def: dict, result: ValidationResult
):
    """Validates ensures clauses."""
    if not isinstance(ensures_def, list):
        result.add_error(
            f"Ensures clauses for '{entity_name}' must be a list",
            field_path=f"entities.{entity_name}.ensures",
        )
        return
    ...
    for idx, rule in enumerate(ensures_def):
        ...
        if not all(k in rule for k in required_keys):
            result.add_error(
                f"Ensures rule #{idx} missing required keys (name, field, constraint, value)",
                field_path=path,
            )
        ...
        if "field" in rule and rule["field"] not in fields_def:
            result.add_error(
                f"Ensures rule '{rule.get('name', idx)}' references unknown field '{rule['field']}'",
                field_path=path,
            )
...
def _validate_entity(entity_name: str, entity_def: Any, result: ValidationResult):
    ...
    if not isinstance(entity_def, dict):
        result.add_error(
            f"Definition for entity '{entity_name}' must be a dictionary",
            field_path=f"entities.{entity_name}",
        )
        return

    if "fields" not in entity_def:
        result.add_error(
            f"Entity '{entity_name}' missing 'fields' section", field_path=f"entities.{entity_name}"
        )
        return

    if not isinstance(entity_def["fields"], dict):
        result.add_error(
            f"Fields for entity '{entity_name}' must be a dictionary",
            field_path=f"entities.{entity_name}.fields",
        )
        return
...
def _validate_field(entity_name: str, field_name: str, field_def: Any, result: ValidationResult):
    ...
    if not isinstance(field_def, dict):
        result.add_error(
            f"Definition for field '{field_name}' must be a dictionary", field_path=path
        )
        return
    ...
    if "enum" in field_def:
        if "type" in field_def and field_def["type"] != "enum":
            result.add_error(
                f"Field '{field_name}' has enum values but type is '{field_def['type']}'",
                field_path=path,
            )
        if not isinstance(field_def["enum"], list):
            result.add_error(
                f"Enum values for field '{field_name}' must be a list", field_path=path
            )
    ...
    if any(k in field_def for k in numeric_constraints):
        if field_type not in ("int", "decimal") and "enum" not in field_def:
            ...
            if field_type in ("uuid", "str", "bool"):
                result.add_error(
                    f"Numeric constraints used on non-numeric field '{field_name}'", field_path=path
                )


def _validate_interface(
    interface_name: str, interface_def: Any, defined_entities: set, result: ValidationResult
):
                ...
                for param_name, param_type in method_def["params"].items():
                    ...
                    if isinstance(param_type, str):
                        ...
                        if param_type[0].isupper() and param_type not in defined_entities:
                            result.add_error(
                                f"Undefined entity reference '{param_type}' in method '{method_name}'",
                                field_path=f"interfaces.{interface_name}.methods.{method_name}.params.{param_name}",
                            )
