...
class TextFormatter(OutputFormatter):
    ...
    def format(self, violations: list[Violation], target_path: Path) -> str:
        ...
        if not violations:
            ...
            for v in violations:
                output_lines.append(f"  {v.filename}:{v.line_number}: {v.rule_id} - {v.message}")
...
class GitHubFormatter(OutputFormatter):
    ...
    def format(self, violations: list[Violation], target_path: Path) -> str:
        ...
        for v in violations:
            ...
            # Build annotation
            annotation = (
                f"::{severity} file={v.filename},line={v.line_number},title={v.rule_id}::{message}"
            )
...
class JUnitFormatter(OutputFormatter):
    ...
    def format_error(self, message: str) -> str:
        root = ET.Element(
            "testsuites", name="Sheriff Validation", tests="1", failures="0", errors="1"
        )
        testsuite = ET.SubElement(
            root, "testsuite", name="Sheriff", tests="1", failures="0", errors="1"
        )
        testcase = ET.SubElement(
            testsuite, "testcase", name="ConfigurationError", classname="sheriff.config"
        )
