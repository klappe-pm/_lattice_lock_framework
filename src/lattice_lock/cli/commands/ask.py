...
@click.option(
    "--model", 
    "-m", 
    help="Force a specific model ID (e.g., 'gpt-4o', 'claude-3-5-sonnet')"
)
@click.option(
    "--task-type", 
    "-t", 
...
@click.option(
    "--compare", 
    is_flag=True, 
    help="Compare results from multiple high-ranking models"
)
@click.pass_context
def ask_command(ctx, prompt: str, model: Optional[str], task_type: Optional[str], compare: bool):
...
async def _ask_async(prompt: str, model_id: Optional[str], task_type_str: Optional[str], compare: bool, verbose: bool):
    console = Console()
        ...
        try:
            ...
            if verbose:
                ...
                if hasattr(response.usage, "prompt_tokens"):
                    meta_table.add_row("Tokens", f"I: {response.usage.prompt_tokens} / O: {response.usage.completion_tokens}")
                    meta_table.add_row("Cost", f"${response.usage.cost:.4f}")
                
                console.print(Panel(meta_table, title="Metadata", border_style="dim"))
            
            console.print(Panel(response.content, title=f"Response ({response.model})", border_style="green"))
...
async def _handle_compare(orchestrator: ModelOrchestrator, prompt: str, task_type: Optional[TaskType], console: Console, verbose: bool):
    ...
    console.print(f"[bold]Comparing {len(top_models)} models for task: {requirements.task_type.name}[/bold]\n")
    ...
    with Progress(
        ...
        p_task = progress.add_task(description="Awaiting responses from all models...", total=len(tasks))
    ...
    for i, response in enumerate(responses):
        ...
        if isinstance(response, Exception):
            console.print(Panel(f"[red]Failed:[/red] {str(response)}", title=f"Model: {m_id}", border_style="red"))
        else:
