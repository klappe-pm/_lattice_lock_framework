...
- Requirement 2
"""
        result = analyzer.analyze_content(content, file_format="markdown")
        assert result is not None
        assert len(result.requirements) >= 2
    ...
    def test_analyze_content_with_format_detection(self) -> None:
        ...
        json_content = json.dumps({"title": "JSON", "phases": []})
        # Explicitly specify json format to test JSON parsing
...
class TestSpecAnalyzerWithSampleSpecs:
    """Tests using sample specifications from the framework."""

    def test_analyze_framework_spec(self) -> None:
        """Test analyzing the framework specification if available."""
        spec_path = Path("specifications/lattice_lock_framework_specifications.md")
    ...
    def test_analyze_versioning_spec(self) -> None:
        """Test analyzing the versioning specification if available."""
        spec_path = Path("specifications/lattice_lock_versioning_strategy.md")
        if not spec_path.exists():
...
class TestSpecAnalyzerLLMIntegration:
    ...
    def test_merge_results(self) -> None:
        ...
        base = SpecificationAnalysis(
            phases=[Phase(name="Phase 1")],
            requirements=[Requirement(id="REQ-001", description="Base req")],
        )

        llm_data: dict[str, Any] = {
            "phases": [
                {"name": "Phase 2", "description": "New phase"},
            ],
            "components": [
                {"name": "NewComponent", "layer": "application"},
            ],
            "requirements": [
                {"id": "REQ-002", "description": "LLM req", "type": "functional"},
            ],
            "constraints": [
                {"id": "CON-001", "description": "LLM constraint"},
            ],
        }
    ...
    def test_merge_results_no_duplicates(self) -> None:
        ...
        llm_data: dict[str, Any] = {
            "phases": [
                {"name": "Phase 1", "description": "Same phase"},
            ],
            "requirements": [
                {"id": "REQ-001", "description": "Same req"},
            ],
        }
