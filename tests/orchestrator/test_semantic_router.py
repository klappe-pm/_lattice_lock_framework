...
async def test_semantic_router_calls_client():
    ...
    mock_client.chat_completion.return_value = APIResponse(
        content="DEBUGGING", model="gpt-4o-mini", provider="openai", usage={}, latency_ms=100
    )
...
async def test_semantic_router_fallback_to_general():
    mock_client = AsyncMock()
    mock_client.chat_completion.return_value = APIResponse(
        content="I am not sure", model="gpt-4o-mini", provider="openai", usage={}, latency_ms=100
    )
...
async def test_task_analyzer_engages_semantic_router():
    mock_client = AsyncMock()
    mock_client.chat_completion.return_value = APIResponse(
        content="CODE_GENERATION", model="gpt-4o-mini", provider="openai", usage={}, latency_ms=100
    )
