...
async def test_run_pipeline_basic(chain_orch, mock_orchestrator):
    ...
    async def mock_resp1(*args, **kwargs):
        return APIResponse(content="Response 1", model="m1", provider="p1", usage=TokenUsage(0,0,0,0.1), latency_ms=10)
    
    async def mock_resp2(*args, **kwargs):
        return APIResponse(content="Response 2", model="m2", provider="p2", usage=TokenUsage(0,0,0,0.2), latency_ms=10)
