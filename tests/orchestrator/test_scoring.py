...
from lattice_lock.orchestrator.types import ModelCapabilities, TaskRequirements, TaskType, ModelProvider
...
def test_score_context_failure(mock_config, sample_model):
    ...
    req = TaskRequirements(
        task_type=TaskType.GENERAL,
        min_context=20000 # Larger than model's 10k
    )
...
def test_score_vision_failure(mock_config, sample_model):
    ...
    blind_model = ModelCapabilities(
        name="blind", api_name="blind", provider=ModelProvider.OPENAI,
        context_window=10000, supports_vision=False,
        reasoning_score=10, coding_score=10, speed_rating=10, 
...
def test_priority_impact(mock_config, sample_model):
    ...
    smart_slow_model = ModelCapabilities(
        name="smart", api_name="smart", provider=ModelProvider.OPENAI, context_window=10000,
        reasoning_score=99, coding_score=99, speed_rating=1, input_cost=50.0, output_cost=50.0
    )
