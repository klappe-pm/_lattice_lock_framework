...
MODELS = ["gpt-4", "gpt-3.5-turbo", "claude-3-opus", "claude-3-sonnet", "gemini-pro", "gemini-ultra", "llama-3-70b", "mixtral-8x7b", "anthropic.claude-v2", "meta.llama3", "mistral-large", "command-r", "gpt-4-turbo", "claude-3-haiku", "gemini-1.5-pro"]
SCENARIOS = [
    "basic_chat", "long_context", "system_prompt_override", "json_mode", "tool_use_single",
    "tool_use_chain", "high_temp", "low_temp", "top_p_sampling", "max_tokens_limit",
    "error_401_auth", "error_429_rate_limit", "error_500_server", "error_503_unavailable", "timeout_error",
    "empty_response", "malformed_json_response", "pii_injection_attempt", "prompt_injection_attempt", "fallback_required"
]
...
MODEL_PROVIDER_MAP = {
    "gpt-4": "openai", "gpt-3.5-turbo": "openai", "gpt-4-turbo": "openai",
    "claude-3-opus": "anthropic", "claude-3-sonnet": "anthropic", "claude-3-haiku": "anthropic",
    "gemini-pro": "google", "gemini-ultra": "google", "gemini-1.5-pro": "google",
    "anthropic.claude-v2": "bedrock", "meta.llama3": "bedrock", "mistral-large": "bedrock",
    "llama-3-70b": "local", "mixtral-8x7b": "local", "command-r": "local"
}
...
async def test_orchestrator_e2e(mock_aiohttp_session, mock_response_factory, provider, model_id, scenario):
    """
    ...
    # B) For Bedrock (uses boto3)
        ...
        with unittest.mock.patch("boto3.client", return_value=mock_boto_client):
            await _run_orchestrator_test(orchestrator, provider, model_id, scenario, status, exception_to_raise)
            return
...
async def _run_orchestrator_test(orchestrator, provider, model_id, scenario, status, exception_to_raise):
    ...
    try:
        ...
        # --- Assertions ---
        if status == 200 and not exception_to_raise and "malformed" not in scenario and "empty" not in scenario:
        ...
        if "fallback" in scenario or status != 200 or exception_to_raise or "malformed" in scenario or "empty" in scenario:
