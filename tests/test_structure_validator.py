...
def test_missing_required_directory(temp_repo):
    shutil.rmtree(os.path.join(temp_repo, 'agent_definitions'))
    result = validate_repository_structure(temp_repo)
    assert not result.valid
    assert any("Missing required root directory: agent_definitions/" in e.message for e in result.errors)
...
def test_agent_definition_naming(temp_repo):
    ...
    assert any("must start with category prefix 'test_agent_'" in msg for msg in error_messages), f"Errors: {error_messages}"

def test_file_naming_edge_cases():
    # Test file naming directly
    res = validate_file_naming("/path/to/repo/agent_definitions/cat/cat_agent_def.yaml", "/path/to/repo")
    assert not res.valid
    assert any("must end with '_definition.yaml'" in e.message for e in res.errors)

    res = validate_file_naming("/path/to/repo/agent_definitions/cat/cat_def_definition.yaml", "/path/to/repo")
    ...
    res = validate_file_naming("/path/to/repo/agent_definitions/cat/other_definition.yaml", "/path/to/repo")
    assert not res.valid
